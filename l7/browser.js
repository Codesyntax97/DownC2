...
async function visitPage(originalUrl) {
  ...
  // Set timer untuk menghentikan proses setelah waktu selesai
  const timeout = setTimeout(async () => {
    console.log(`[+] Time is up! Stopping browser and flood process.`);
    await browser.close();
    process.exit(0); // Menghentikan script
  }, time * 1000);
    ...
    if (isCFProtect) {
      ...
      const intervalId = setInterval(async () => {
        ...
        if (stillProtected) {
          ...
          console.log(
            `[+] Starting Flooding To ${target} For ${time} Seconds`
          );
